...
import { FileText, Upload, Cloud, MoveRight, Globe } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[160px]">
      ...
      >
        ...
        <div className="text-left">
          <h3 className="text-sm font-bold text-on-surface">{t("knowledge.scrapeWebsite")}</h3>
          <p className="mt-1 text-xs text-on-surface-variant/70">{t("knowledge.websiteUrl")}</p>
        </div>
        <MoveRight className="absolute bottom-6 right-6 h-4 w-4 text-primary opacity-0 -translate-x-2 transition-all group-hover:opacity-100 group-hover:translate-x-0" />
      </button>
      ...
      <button
        ...
        className="group relative flex flex-col justify-between overflow-hidden rounded-xl bg-primary-container p-6 transition-all hover:brightness-105 active:scale-[0.98] md:col-span-1"
      >
