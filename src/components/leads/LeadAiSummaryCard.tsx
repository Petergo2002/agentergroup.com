...
function SummaryActionButton({
  ...
  return (
    <button
      ...
      className="inline-flex h-9 shrink-0 items-center gap-2 rounded-lg border border-outline-variant/20 bg-surface-container-lowest px-3 text-xs font-semibold text-on-surface transition-colors hover:border-primary/30 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 disabled:cursor-wait disabled:opacity-60"
    >
...
export function LeadAiSummaryCard({
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-primary/15 bg-primary/[0.035] p-5 sm:p-6"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div>
            <h3 id={titleId} className="text-sm font-bold text-on-surface">
      ...
      {!summary ? (
        ...
        <div className="mt-5 space-y-5">
              ...
              <dl className="mt-3 grid gap-x-5 gap-y-3 sm:grid-cols-2">
                {details.map(([label, value]) => (
                  <div key={label} className="min-w-0">
                    <dt className="text-[11px] text-on-surface-variant/55">{label}</dt>
                    <dd className="mt-0.5 break-words text-sm font-medium text-on-surface">
                      {value}
                    </dd>
                  </div>
                ))}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest/70 p-4">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {content.missingInformation.map((item) => (
                  <span
                    key={item}
                    className="rounded-lg border border-outline-variant/15 bg-surface-container-lowest px-2.5 py-1.5 text-xs text-on-surface-variant"
                  >
                    {item}
                  </span>
                ))}
          ...
          {summary.generated_at ? (
            <p className="border-t border-outline-variant/10 pt-3 text-[10px] text-on-surface-variant/45">
