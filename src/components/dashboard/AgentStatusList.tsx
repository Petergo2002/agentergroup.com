...
export function AgentStatusList({ agents, isLoading }: AgentStatusListProps) {
  const getStatusConfig = (agent: AgentRecord) => {
    if (agent.archived_at) return { label: "Archived", classes: "bg-surface-container text-on-surface-variant/60" };
    if (agent.status === "active" && agent.published_version_id) return { label: "Live", classes: "bg-success/10 text-success" };
    if (agent.status === "active") return { label: "Ready", classes: "bg-primary/10 text-primary" };
    if (agent.status === "paused") return { label: "Paused", classes: "bg-surface-container text-on-surface-variant/70" };
    return { label: "Draft", classes: "bg-surface-container-high/40 text-on-surface-variant/40" };
  };
    ...
    <div className="rounded-[2rem] bg-surface-container-low/30 p-8 shadow-[0_24px_60px_rgba(15,23,42,0.06)] ring-1 ring-outline-variant/10">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h2 className="font-headline text-2xl font-bold text-on-surface">Agent Status</h2>
        </div>
        <Link
          href="/agents"
          ...
          agents.slice(0, 6).map((agent) => {
            ...
            return (
              <Link
                ...
                className="group flex items-center justify-between gap-4 rounded-2xl bg-surface-container-lowest/50 p-4 transition-all hover:bg-white hover:shadow-lg hover:shadow-primary/5 hover:ring-1 hover:ring-primary/10"
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-on-surface-variant/60 font-medium">
                      Edited {formatRelativeDate(agent.updated_at)}
                    </p>
                  </div>
                </div>
