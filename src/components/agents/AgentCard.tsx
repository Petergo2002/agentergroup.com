...
export function AgentCard({
  ...
  const getDotColor = (agent: AgentRecord) => {
    ...
    if (agent.surface === "assistant") {
      return agent.status === "active" ? "bg-emerald-500" : "bg-[#CBD5E1]";
    }
  ...
  return (
    <div className="group relative flex flex-col justify-between p-6 rounded-[2rem] bg-surface-container-low/30 ring-1 ring-outline-variant/10 shadow-[0_8px_30px_rgba(15,23,42,0.02)] transition-all hover:bg-white hover:shadow-xl hover:shadow-primary/5 hover:ring-primary/20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className={`h-12 w-12 rounded-2xl bg-surface-container flex items-center justify-center text-primary-orange font-bold text-xl`}>
              {agent.name.charAt(0)}
            </div>
            <span className={`absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full border-2 border-white ${getDotColor(agent)}`} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
            ...
            >
                ...
                {/* Handle */}
                <span
                    className={`absolute top-1 left-1 h-[18px] w-[18px] rounded-full bg-white shadow-[0_2px_4px_rgba(0,0,0,0.1)] transition-all duration-300 ease-in-out ${
      ...
      <p className="text-[13px] leading-relaxed text-on-surface-variant/70 line-clamp-2 mb-8 min-h-[40px]">
        {agent.description || "No specific instructions provided for this agent yet."}
      </p>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-outline-variant/5">
        ...
        <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity translate-x-2 group-hover:translate-x-0 transition-transform">
          <Link
            ...
            className="flex h-9 items-center justify-center rounded-xl bg-on-surface px-4 text-[11px] font-bold text-background hover:opacity-90 transition-all"
          >
            {canEdit || agent.surface === 'widget' ? 'Builder' : 'Open'}
          </Link>
          <Link
