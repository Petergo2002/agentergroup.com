...
- **Cross-Workspace Badge Leak Fixed (18 September 2026):** The sidebar attention badges read `/api/dashboard/latest-activity`, which inferred the workspace from the session and was browser-cached for 15 seconds under a URL identical for every workspace. Switching workspaces therefore showed the previous workspace's lead and question counts. The workspace is now named in the URL and validated against the caller's memberships. **When reviewing any cached GET that infers its workspace, check the URL varies per workspace** — a `Cache-Control` with a shared URL turns a stale-context window into a visible cross-tenant number.
